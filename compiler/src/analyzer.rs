use crate::parser::{HTMLChild, HTMLElement, HTMLEndTag, HTMLStartTag};

pub fn analyze_tree(tree: &HTMLElement) -> Result<(), String> {
    match tree {
        HTMLElement::ElementWithChildren {
            start_tag,
            end_tag,
            children,
        } => {
            ...
            if !start_name.to_lowercase().eq(&end_name.to_lowercase()) {
                return Err(format!(
                    "Start and end tag are not equal. Start tag: {}, end tag: {}",
                    start_name, end_name
                ));
            }
...
pub fn analyze_children(children: &[HTMLChild]) -> Result<(), String> {
